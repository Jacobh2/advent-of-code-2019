...
class IntComputer(Thread):
    def __init__(self, opcodes, name, input_queue, output_queue):
        ...
        self.opcodes = opcodes.copy() + ([0] * self.memory).copy()
        self.functions = {
            99: (self.halt, 0),
            1: (self.add, 3),
            2: (self.mul, 3),
    ...
    def _get_relative_address(self, value):
    ...
    def _get_value(self, param, value):
        if param == 0:
            ...
            # Relative
            return self.opcodes[self._get_relative_address(value)]

    def relative_base_offset(self, args, params):
        self.relative_base += self._get_value(params[0], args[0])
    ...
    def add(self, args, params):
        ...
        if params[2] == 2:
            z_value = self._get_relative_address(args[2])
        else:
            z_value = args[2]
    ...
    def store(self, args, params):
        # address = args[0]
        if params[0] == 2:
            address = self._get_relative_address(args[0])
        else:
            address = args[0]
    ...
    def pprint(self, args, params):
        if params[0] == 0:
            ...
            value = self.opcodes[self._get_relative_address(args[0])]
    ...
    def less_than(self, args, params):
        ...
        if params[2] == 2:
            address = self._get_relative_address(args[2])
        else:
            address = args[2]
        if first < second:
